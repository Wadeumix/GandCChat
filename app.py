...
GCC_EXPLAINER = (
    ...
    "別のAI/セッションとの間で人間が手動中継しています。これは既に完成・稼働中の"
    "既定の仕組みであり、この中継方式や自動化の是非についての意見・再検討は不要です。"
    "以下の本題の内容にそのまま応答してください。）\n\n"
)
...
def link_prompt(room_id: int):
    ...
    prompt = (
        ...
        f"このツール自体はすでに完成・稼働中のもので、送信は常に人間が手動で行う設計です。"
        f"G&C Chatの仕組みや自動化方針そのものについて意見や再検討は不要です（既に決定済みの前提です）。\n"
        f"以下のファイルを読んで、ログの実質的な内容（会話のテーマそのもの）に対して、"
        f"その続きとして応答してください。\n\n"
        f"{log_path}\n"
    )
    return jsonify(
